...
public class TrialFilter {
    ...
    Stream<Trial> filterByCommonCropName(Stream<Trial> trialStream, String commonCropName) {
        return trialStream.filter(trial -> areParametersNonNullAndEquals(trial.getCommonCropName(), commonCropName));
    }

    Stream<Trial> filterByProgramDbId(Stream<Trial> trialStream, String programDbId) {
        return trialStream.filter(trial -> areParametersNonNullAndEquals(trial.getProgramDbId(), programDbId));
    }
    ...
    Stream<Trial> filterByActive(Stream<Trial> trialStream, String active) {
        return trialStream.filter(trial -> areParametersNonNullAndEquals(Boolean.toString(trial.isActive()), active.toLowerCase()));
    }
}
