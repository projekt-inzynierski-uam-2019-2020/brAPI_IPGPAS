...
def convert():
    ...
    for dir_ in directories:
            ...
            with open(s_file, encoding='utf8') as tsv_file:
                ...
                try:
                    ...
                    for row in reader:
                        ...
                        if not file_name in isatab_json['Samples']:
                            isatab_json['Samples'][file_name] = {}
                
                        isatab_json['Samples'][file_name][sample_name] = germplasm
        ...
        for d_file in isatab_files_dict['d_files']:
            ...
            with open(d_file, encoding='utf8') as tsv_file:
                ...
                try:
                    ...
                    assays = isatab_json['Assays'][file_name]   # dict
        ...
        for tdf_file in isatab_files_dict['tdf_file']:
            ...
            with open(tdf_file, encoding='utf8') as tsv_file:
        ...
        for s_file, samples_dict in isatab_json['Samples'].items():

            if s_file not in isatab_json['FinalData']:
                isatab_json['FinalData'][s_file] = {}

            for sample1, germplasm in samples_dict.items():
                ...
                for d_file in isatab_json['Assays']:
                    ...
                    for sample2, data in isatab_json['Assays'][d_file].items():
                        ...
                        if sample1 == sample2:
                            ...
                            if germplasm not in isatab_json['FinalData'][s_file]:
                                isatab_json['FinalData'][s_file][germplasm] = []

                            isatab_json['FinalData'][s_file][germplasm].append(data)
